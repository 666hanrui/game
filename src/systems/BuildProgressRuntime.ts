...
const ROUTE_META: Record<BuildRouteId, { name: string; color: string; starter: string; formed: string; complete: string }> = {
  ...
  blade: { name: "刃系", color: "#ffcc80", starter: "刃系起势", formed: "刃系成型", complete: "刃系大成" },
...
export function buildBuildProgress(skills: Skill[], weaponId?: string | null): BuildProgressSnapshot {
  ...
  for (const skill of skills) {
    ...
    if (skill.school === "martial" && route !== "martial") routePoints.set("martial", (routePoints.get("martial") ?? 0) + weight * 0.28);
    if (skill.school === "magic" && route !== "magic") routePoints.set("magic", (routePoints.get("magic") ?? 0) + weight * 0.18);
    if (skill.school === "tech" && route !== "tech") routePoints.set("tech", (routePoints.get("tech") ?? 0) + weight * 0.18);
  }
  ...
  return { primary, routes, activeSynergyNames: synergySnapshot.activeSynergies.map((synergy) => synergy.name), activeSynergyCount: synergySnapshot.activeSynergies.length, hybrid, label, color, score };
}
...
function routeFromWeapon(weaponId: string): BuildRouteId {
  if (weaponId === "bow" || weaponId === "crossbow" || weaponId === "mechanical_crossbow") return "bow";
  if (weaponId === "spear") return "spear";
  if (weaponId === "flying_blade" || weaponId === "sword" || weaponId === "greatsword" || weaponId === "dagger" || weaponId === "katana" || weaponId === "chakram" || weaponId === "chain_sickle" || weaponId === "claw" || weaponId === "plasma_blade") return "blade";
  if (weaponId === "mace" || weaponId === "battle_axe" || weaponId === "shield_blade") return "mace";

  const weapon = getWeapon(weaponId);
  if (!weapon) return "neutral";
  if (weapon.school === "magic") return "magic";
  if (weapon.school === "tech") return "tech";
  if (weapon.school === "martial") return "martial";
  return "neutral";
}
...
function routeFromSkill(skill: Skill): BuildRouteId {
  ...
  if (skill.tags?.includes("blade") || skill.tags?.includes("slash") || skill.tags?.includes("dagger")) return "blade";
  if (skill.tags?.includes("mace") || skill.tags?.includes("heavy") || skill.tags?.includes("slam")) return "mace";
...
function pickHybrid(primary: RouteProgress, activeIds: Set<string>, synergyCount: number): HybridBuildProgress | null {
  ...
  if (primary.id === "bow" && formed && activeIds.has("lightning_split_arrow")) return { id: "thunder_arrow_array", name: complete ? "雷羽箭阵" : "雷羽成型", color: "#90caf9", description: "多重箭与雷纹共鸣产生连锁电弧，箭阵进入杂交成型。", powerLevel: complete ? 3 : 2 };
  if (primary.id === "mace" && formed && activeIds.has("fire_mace_quake")) return { id: "lava_quake", name: complete ? "熔岩地裂" : "熔岩重击", color: "#ff7043", description: "钝器地裂与炎脉联动，重击会形成熔岩裂缝感。", powerLevel: complete ? 3 : 2 };
  if (primary.id === "spear" && formed && activeIds.has("lightning_spear_beam")) return { id: "thunder_spear_array", name: complete ? "雷贯枪阵" : "雷贯枪芒", color: "#80deea", description: "枪芒与雷纹共鸣，贯穿后产生电链。", powerLevel: complete ? 3 : 2 };
  if (primary.id === "blade" && formed && activeIds.has("poison_blade_whirl")) return { id: "poison_blade_storm", name: complete ? "毒刃风暴" : "毒刃回旋", color: "#81c784", description: "刃系路线与毒蚀联动，回旋路径带腐蚀反馈。", powerLevel: complete ? 3 : 2 };
  if (primary.id === "tech" && formed && activeIds.has("arcane_drone")) return { id: "arcane_drone_swarm", name: complete ? "奥术无人机群" : "奥术无人机", color: "#b39ddb", description: "无人机火力与奥术星环联动，科技弹体获得法术感。", powerLevel: complete ? 3 : 2 };
  if (primary.id === "bow" && formed && activeIds.has("ice_frost_arrow")) return { id: "frost_arrow_rain", name: complete ? "霜痕箭雨" : "霜痕箭路", color: "#80deea", description: "冰霜箭与霜息凝痕联动，箭路减速反馈更明显。", powerLevel: complete ? 3 : 2 };
  if (synergyCount >= 3 && formed) return { id: "hybrid_mastery", name: complete ? "杂交大成" : "杂交成型", color: "#ffd54f", description: "多条隐藏联动同时激活，体系已进入复合构筑。", powerLevel: complete ? 3 : 2 };
  return null;
}
