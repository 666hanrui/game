...
export class HubCampPanel {
  ...
  update(input: Input, dt: number, _w: number, _h: number): HubCampInteraction | null {
    ...
    const speed = 245;
    this.player.x = this.clamp(this.player.x + move.x * speed * dt, 46, CAMP_W - 46);
    this.player.y = this.clamp(this.player.y + move.y * speed * dt, 72, CAMP_H - 48);
  ...
  private getView(w: number, h: number): CampView {
    const scale = Math.max(0.72, Math.min(1.18, Math.min(w / 1220, h / 800)));
    const camX = this.clamp(this.camera.x, w / (2 * scale), CAMP_W - w / (2 * scale));
    const camY = this.clamp(this.camera.y, h / (2 * scale), CAMP_H - h / (2 * scale));
    ...
    return {
      ...
      ox,
      oy,
      toScreen: (x: number, y: number) => ({ x: x * scale + ox, y: y * scale + oy }),
      toWorld: (x: number, y: number) => ({ x: (x - ox) / scale, y: (y - oy) / scale }),
    };
  }
  ...
  private drawFallbackGround(ctx: CanvasRenderingContext2D): void {
    ...
    ctx.lineCap = "round";
    ctx.strokeStyle = "rgba(154, 103, 55, 0.68)";
    ctx.lineWidth = 30;
    for (const b of CAMP_BUILDINGS) {
      ctx.beginPath();
      ctx.moveTo(CAMP_W / 2, CAMP_H / 2 + 110);
      ctx.quadraticCurveTo((CAMP_W / 2 + b.interactPoint.x) / 2, (CAMP_H / 2 + b.interactPoint.y) / 2 + 24, b.interactPoint.x, b.interactPoint.y);
      ctx.stroke();
    }
    ctx.fillStyle = "rgba(255, 224, 130, 0.42)";
    ctx.beginPath();
    ctx.ellipse(CAMP_W / 2, CAMP_H / 2 + 110, 116, 78, 0, 0, Math.PI * 2);
    ctx.fill();
  }
  ...
  private drawImage(ctx: CanvasRenderingContext2D, img: HTMLImageElement, x: number, y: number, w: number, h: number, glow: string | null): void {
    const oldSmoothing = ctx.imageSmoothingEnabled;
    ctx.imageSmoothingEnabled = false;
    ctx.save();
    if (glow) {
  ...
  private drawPlayer(ctx: CanvasRenderingContext2D, view: CampView): void {
    ...
    ctx.beginPath();
    ctx.ellipse(p.x, p.y + 9 * s, 14 * s, 6 * s, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "#4fc3f7";
    ctx.strokeStyle = "rgba(255,255,255,0.9)";
    ctx.lineWidth = 2 * s;
    ctx.beginPath();
    ctx.ellipse(p.x, p.y - 6 * s, 12 * s, 18 * s, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.stroke();
    ctx.beginPath();
    ctx.arc(p.x, p.y - 28 * s, 12 * s, 0, Math.PI * 2);
    ctx.fill();
    ctx.stroke();
    ctx.fillStyle = "rgba(0,0,0,0.58)";
    ctx.beginPath();
    ctx.arc(p.x - 4 * s, p.y - 30 * s, 2 * s, 0, Math.PI * 2);
    ctx.arc(p.x + 4 * s, p.y - 30 * s, 2 * s, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "#fff";
    ctx.font = `bold ${Math.max(10, 12 * s)}px monospace`;
    ctx.textAlign = "center";
    ctx.fillText("你", p.x, p.y + 30 * s);
    ctx.restore();
  }
  ...
  private drawStartFallback(ctx: CanvasRenderingContext2D, w: number, h: number): void {
    ...
    this.panel(ctx, r.x, r.y, r.w, r.h, 18, g as unknown as string, "rgba(255,202,40,0.88)", 2.4);
    ctx.fillStyle = "#fff8e1";
