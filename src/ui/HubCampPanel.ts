...
const MODULE_ACCENT: Record<HubModuleId, string> = {
  expedition: "#ffca28", talents: "#ce93d8", economyStorage: "#ffd54f", workshop: "#ffcc80",
  apothecary: "#81c784", quests: "#e1f5fe", crafting: "#b3e5fc", storage: "#90caf9",
  loot: "#ffd54f", map: "#ff8a65", archive: "#b0bec5",
};
...
export class HubCampPanel {
  ...
  update(input: Input, dt: number, _w: number, _h: number): HubCampInteraction | null {
    ...
    if (pressed && this.activeBuilding) { this.interactFlash = 0.5; return this.getBuildingInteraction(this.activeBuilding); }
    return null;
  }
  ...
  private loadHubImages(): void {
    for (const [key, src] of Object.entries(HUB_ART_PATHS) as [HubArtKey, string][]) {
      const img = new Image(); img.src = src; this.hubImages.set(key, img);
    }
  }
  ...
  private getView(w: number, h: number): CampView {
    ...
    return { scale, ox, oy, toScreen: (x, y) => ({ x: x * scale + ox, y: y * scale + oy }), toWorld: (x, y) => ({ x: (x - ox) / scale, y: (y - oy) / scale }) };
  }
  ...
  private drawFallbackGround(ctx: CanvasRenderingContext2D): void {
    ...
    g.addColorStop(0, "#74a85a"); g.addColorStop(0.58, "#66954f"); g.addColorStop(1, "#4d7d47");
    ctx.fillStyle = g; ctx.fillRect(0, 0, CAMP_W, CAMP_H);
  }

  private drawSceneLayer(ctx: CanvasRenderingContext2D, view: CampView): void {
    const items: SceneItem[] = CAMP_BUILDINGS.map((b) => ({ depth: b.depthY, draw: () => this.drawBuildingBack(ctx, view, b) }));
    items.push({ depth: this.player.y + 12, draw: () => this.drawPlayer(ctx, view) });
    items.sort((a, b) => a.depth - b.depth);
    for (const item of items) item.draw();
  }

  private drawFrontOverlayLayer(ctx: CanvasRenderingContext2D, view: CampView): void { for (const building of CAMP_BUILDINGS) this.drawBuildingFront(ctx, view, building); }
  ...
  private drawImage(ctx: CanvasRenderingContext2D, img: CanvasImageSource, x: number, y: number, w: number, h: number, glow: string | null): void {
    const oldSmoothing = ctx.imageSmoothingEnabled; ctx.imageSmoothingEnabled = false; ctx.save();
    if (glow) { ctx.shadowColor = glow; ctx.shadowBlur = 14; }
    ctx.drawImage(img, x, y, w, h); ctx.restore(); ctx.imageSmoothingEnabled = oldSmoothing;
  }
  ...
  private drawFallbackBuilding(ctx: CanvasRenderingContext2D, view: CampView, b: CampBuilding, active: boolean, selected: boolean): void {
    const p = view.toScreen(b.x, b.y); const w = b.w * view.scale; const h = b.h * view.scale;
    ctx.save(); ctx.fillStyle = active ? this.rgba(b.color, 0.42) : selected ? this.rgba(b.color, 0.28) : "rgba(92, 67, 44, 0.95)";
    ctx.strokeStyle = active ? b.color : "rgba(78, 48, 25, 0.9)"; ctx.lineWidth = active ? 3 : 1.5;
    this.roundRect(ctx, p.x + w * 0.15, p.y + h * 0.33, w * 0.7, h * 0.45, 10 * view.scale); ctx.fill(); ctx.stroke();
    ctx.fillStyle = this.rgba(b.color, 0.65); ctx.beginPath(); ctx.moveTo(p.x + w * 0.06, p.y + h * 0.36); ctx.lineTo(p.x + w * 0.5, p.y + h * 0.08); ctx.lineTo(p.x + w * 0.94, p.y + h * 0.36); ctx.closePath(); ctx.fill(); ctx.stroke();
    ctx.fillStyle = "#fff8e1"; ctx.font = `bold ${Math.max(14, 24 * view.scale)}px monospace`; ctx.textAlign = "center"; ctx.fillText(b.icon, p.x + w / 2, p.y + h * 0.62); ctx.restore();
  }

  private drawBuildingShadow(ctx: CanvasRenderingContext2D, view: CampView, b: CampBuilding, active: boolean): void {
    const fp = b.footprint; const p = view.toScreen(fp.x + fp.w / 2, fp.y + fp.h / 2);
    ctx.fillStyle = active ? "rgba(0,0,0,0.28)" : "rgba(0,0,0,0.18)"; ctx.beginPath(); ctx.ellipse(p.x, p.y, fp.w * view.scale * 0.56, fp.h * view.scale * 0.58, 0, 0, Math.PI * 2); ctx.fill();
  }

  private drawPlayer(ctx: CanvasRenderingContext2D, view: CampView): void {
    const p = view.toScreen(this.player.x, this.player.y); const s = view.scale;
    ctx.save(); ctx.fillStyle = "rgba(0,0,0,0.32)"; ctx.beginPath(); ctx.ellipse(p.x, p.y + 9 * s, 15 * s, 6 * s, 0, 0, Math.PI * 2); ctx.fill();
    if (this.humanWalkSheet.complete && this.humanWalkSheet.naturalWidth > 0) this.drawHumanWalkSheet(ctx, this.humanWalkSheet, p.x, p.y, s); else this.drawFallbackPlayer(ctx, p.x, p.y, s);
    ctx.restore();
  }

  private drawHumanWalkSheet(ctx: CanvasRenderingContext2D, sheet: HTMLImageElement, x: number, y: number, scale: number): void {
    const cols = 4; const rows = 4; const frameW = sheet.naturalWidth / cols; const frameH = sheet.naturalHeight / rows;
    const rowMap: Record<HubAvatarDirection, number> = { down: 0, up: 1, left: 2, right: 3 };
    const row = rowMap[this.avatarDirection]; const frame = this.avatarMoving ? Math.floor(this.avatarAnimTime * 8) % 4 : 0;
    const drawW = 58 * scale; const drawH = 58 * scale; const oldSmoothing = ctx.imageSmoothingEnabled; ctx.imageSmoothingEnabled = false;
  ...
  private drawFallbackPlayer(ctx: CanvasRenderingContext2D, x: number, y: number, s: number): void {
    ctx.fillStyle = "#ffcc80"; ctx.strokeStyle = "rgba(68,42,28,0.95)"; ctx.lineWidth = 2 * s;
    ctx.beginPath(); ctx.ellipse(x, y - 8 * s, 11 * s, 17 * s, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
    ctx.fillStyle = "#6d4c41"; ctx.beginPath(); ctx.arc(x, y - 30 * s, 12 * s, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
    ctx.fillStyle = "#fff"; ctx.font = `bold ${Math.max(10, 12 * s)}px monospace`; ctx.textAlign = "center"; ctx.fillText("你", x, y + 30 * s);
  }

  private drawInteractionHints(ctx: CanvasRenderingContext2D, view: CampView): void {
    if (!this.activeBuilding) return; const p = view.toScreen(this.activeBuilding.interactPoint.x, this.activeBuilding.interactPoint.y - 44);
    const w = 92 * view.scale; const h = 28 * view.scale; ctx.fillStyle = "rgba(53, 34, 20, 0.86)"; ctx.strokeStyle = this.activeBuilding.color; ctx.lineWidth = 1.5;
    this.roundRect(ctx, p.x - w / 2, p.y - h / 2, w, h, 10 * view.scale); ctx.fill(); ctx.stroke();
    ctx.fillStyle = this.interactFlash > 0 ? "#ffffff" : "#ffeb3b"; ctx.font = `bold ${Math.max(10, 12 * view.scale)}px monospace`; ctx.textAlign = "center"; ctx.fillText("E 交互", p.x, p.y + 4 * view.scale);
  }
  ...
  private drawEdgeFogOverlay(ctx: CanvasRenderingContext2D, w: number, h: number): void {
    const fog = Math.max(110, Math.min(220, Math.min(w, h) * 0.16)); const color = "18,35,18"; ctx.save();
    let g = ctx.createLinearGradient(0, 0, fog, 0); g.addColorStop(0, `rgba(${color},0.88)`); g.addColorStop(1, `rgba(${color},0)`); ctx.fillStyle = g; ctx.fillRect(0, 0, fog, h);
    g = ctx.createLinearGradient(w - fog, 0, w, 0); g.addColorStop(0, `rgba(${color},0)`); g.addColorStop(1, `rgba(${color},0.92)`); ctx.fillStyle = g; ctx.fillRect(w - fog, 0, fog, h);
    g = ctx.createLinearGradient(0, 0, 0, fog); g.addColorStop(0, `rgba(${color},0.72)`); g.addColorStop(1, `rgba(${color},0)`); ctx.fillStyle = g; ctx.fillRect(0, 0, w, fog);
    g = ctx.createLinearGradient(0, h - fog, 0, h); g.addColorStop(0, `rgba(${color},0)`); g.addColorStop(1, `rgba(${color},0.82)`); ctx.fillStyle = g; ctx.fillRect(0, h - fog, w, fog);
    g = ctx.createRadialGradient(w / 2, h / 2, Math.min(w, h) * 0.32, w / 2, h / 2, Math.max(w, h) * 0.72); g.addColorStop(0, "rgba(0,0,0,0)"); g.addColorStop(1, "rgba(0,0,0,0.28)"); ctx.fillStyle = g; ctx.fillRect(0, 0, w, h); ctx.restore();
  }

  private drawBuildingName(ctx: CanvasRenderingContext2D, view: CampView, b: CampBuilding): void {
    const p = view.toScreen(b.interactPoint.x, b.interactPoint.y + 18); const w = Math.max(86, b.name.length * 16) * view.scale; const h = 24 * view.scale;
    ctx.fillStyle = "rgba(47, 31, 18, 0.82)"; ctx.strokeStyle = this.rgba(b.color, 0.82); ctx.lineWidth = 1.5; this.roundRect(ctx, p.x - w / 2, p.y - h / 2, w, h, 8 * view.scale); ctx.fill(); ctx.stroke();
    ctx.fillStyle = "#fff8e1"; ctx.font = `bold ${Math.max(10, 13 * view.scale)}px monospace`; ctx.textAlign = "center"; ctx.fillText(b.name, p.x, p.y + 4 * view.scale);
  }
  ...
  private drawHud(ctx: CanvasRenderingContext2D, w: number): void {
    ...
    ctx.fillStyle = "#ffca28"; ctx.font = "bold 26px monospace"; ctx.textAlign = "left"; ctx.fillText("远征营地", 44, 54);
    ctx.fillStyle = "rgba(255,255,255,0.68)"; ctx.font = "12px monospace"; ctx.fillText("WASD 移动 · 靠近建筑门口按 E 交互 · 远征城门开始战斗", 46, 79);
  }

  private drawInteractionPanel(ctx: CanvasRenderingContext2D, w: number, h: number): void {
    const module = HUB_MODULES.find((m) => m.id === this.selectedModule) ?? HUB_MODULES[0]; const building = CAMP_BUILDINGS.find((b) => b.id === this.selectedModule);
    const panelW = Math.min(448, w - 40); const panelH = 140; const x = 20; const y = h - panelH - 22; const color = MODULE_ACCENT[this.selectedModule];
    this.panel(ctx, x, y, panelW, panelH, 18, "rgba(39, 27, 17, 0.84)", this.rgba(color, 0.52)); ctx.textAlign = "left"; ctx.fillStyle = color; ctx.font = "bold 20px monospace"; ctx.fillText(`${building?.icon ?? module.icon} ${building?.name ?? module.name}`, x + 20, y + 34);
    ctx.fillStyle = "rgba(255,255,255,0.76)"; ctx.font = "12px monospace"; this.wrapText(ctx, module.description, x + 20, y + 60, panelW - 40, 18, 3);
    if (building) { const actionDef = getHubActionByModule(building.id); const subPanelId = actionDef ? getHubSubPanelId(actionDef.action) : undefined; ctx.fillStyle = this.interactFlash > 0 ? "#ffeb3b" : "rgba(255,255,255,0.6)"; ctx.font = "bold 12px monospace"; ctx.fillText(`${building.npc}：${building.line} ${actionDef?.action ?? "未配置"}${subPanelId ? ` → ${subPanelId}` : ""}`, x + 20, y + panelH - 22); }
  }

  private drawStartFallback(ctx: CanvasRenderingContext2D, w: number, h: number): void {
    const r = this.startButtonRect; const g = ctx.createLinearGradient(r.x, r.y, r.x + r.w, r.y + r.h); g.addColorStop(0, "rgba(255,202,40,0.36)"); g.addColorStop(1, "rgba(121,85,72,0.68)"); this.panel(ctx, r.x, r.y, r.w, r.h, 18, g, "rgba(255,202,40,0.88)", 2.4);
    ctx.fillStyle = "#fff8e1"; ctx.font = "bold 18px monospace"; ctx.textAlign = "center"; ctx.fillText("开始远征", r.x + r.w / 2, r.y + 33); if (w > 960) { ctx.fillStyle = "rgba(255,255,255,0.5)"; ctx.font = "11px monospace"; ctx.fillText("也可以走到远征城门按 E", r.x + r.w / 2, h - 11); }
  }
  ...
  private findBuildingAtScreen(x: number, y: number): CampBuilding | null { if (!this.lastView) return null; const world = this.lastView.toWorld(x, y); const ordered = [...CAMP_BUILDINGS].sort((a, b) => b.depthY - a.depthY); return ordered.find((b) => this.inRect(world.x, world.y, { x: b.x, y: b.y, w: b.w, h: b.h })) ?? null; }
  private getModuleInteraction(moduleId: HubModuleId): HubCampInteraction | null { return getHubActionByModule(moduleId)?.action ?? null; }
  private getBuildingInteraction(building: CampBuilding): HubCampInteraction | null { return this.getModuleInteraction(building.id); }
  private panel(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, r: number, fill: string | CanvasGradient, stroke: string, lineWidth = 1): void { ctx.fillStyle = fill; ctx.strokeStyle = stroke; ctx.lineWidth = lineWidth; this.roundRect(ctx, x, y, w, h, r); ctx.fill(); ctx.stroke(); }
  private roundRect(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, r: number): void { const rr = Math.min(r, w / 2, h / 2); ctx.beginPath(); ctx.moveTo(x + rr, y); ctx.lineTo(x + w - rr, y); ctx.arcTo(x + w, y, x + w, y + rr, rr); ctx.lineTo(x + w, y + h - rr); ctx.arcTo(x + w, y + h, x + w - rr, y + h, rr); ctx.lineTo(x + rr, y + h); ctx.arcTo(x, y + h, x, y + h - rr, rr); ctx.lineTo(x, y + rr); ctx.arcTo(x, y, x + rr, y, rr); ctx.closePath(); }
  private inRect(x: number, y: number, r: Rect): boolean { return x >= r.x && x <= r.x + r.w && y >= r.y && y <= r.y + r.h; }
  private clamp(value: number, min: number, max: number): number { if (min > max) return value; return Math.max(min, Math.min(max, value)); }
  private rgba(hex: string, alpha: number): string { const clean = hex.replace("#", ""); const r = parseInt(clean.slice(0, 2), 16); const g = parseInt(clean.slice(2, 4), 16); const b = parseInt(clean.slice(4, 6), 16); return `rgba(${r},${g},${b},${alpha})`; }
  private wrapText(ctx: CanvasRenderingContext2D, text: string, x: number, y: number, maxW: number, lineH: number, maxLines: number): void { let line = ""; let cy = y; let lines = 0; for (const char of text) { const test = line + char; if (ctx.measureText(test).width > maxW && line) { ctx.fillText(line, x, cy); lines++; if (lines >= maxLines) return; line = char; cy += lineH; } else line = test; } if (line && lines < maxLines) ctx.fillText(line, x, cy); }
}
